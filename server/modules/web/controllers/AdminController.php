...
class AdminController extends BaseController
{
    ...
    public function actionAuthregect()
    {
        $params = [
            ...
            'studentcertificateindex'=>['url'=>'index.php?r=web/admin/cateedit','status'=>EStudentcertificate::FILED],
            'joinindex'=>['url'=>'index.php?r=web/admin/joinedit','status'=>BJoin::FILED],
        ];
    ...
    public function actionStudentcertificateindex()
    {
        ...
        return $this->renderPartial('studentcertificatelist',['cate'=>$studentcertificate]);
    }
    /**
     * Renders the index view for the module
     * @return string
    ...
    public function actionJoinindex()
    {
        ...
        return $this->renderPartial('joinindex',['join'=>$join]);
    }
    /**
     * Renders the index view for the module
     * @return string
     ...
     * 专业技能管理 - 编辑
     */
    public function actionProfessionaledit()
    {
        $professional = [];
        if(\Yii::$app->request->isPost)
        {
            ...
            if(empty($post))
            {
                self::getFailInfo('技能名称不得为空',$this->method);
            }
            ...
            if(\Yii::$app->request->post('id'))
            {
                ...
                if(!$id)
                {
                    self::getFailInfo('专业技能编辑失败',$this->method);
                }
            }else{
                //添加
                $id = BProfessional::insertProfessional($post);
                if(!$id)
                {
                    self::getFailInfo('专业技能添加失败',$this->method);
                }
            ...
            if ($id)
            {
                $professional = BProfessional::findOne($id);
                if(!$professional)
                {
                    self::getFailInfo('参数错误',$this->method);
                }
        ...
        return $this->renderPartial('professionaledit',['professional'=>$professional,'user'=>$user]);
    }
    /**
     * 状态变更（是否发布到首页）
     */
                ...
                case 'article':
                    {
                        ...
                        {
                            $post = ['id'=>$r,'isRec'=>$isRec];
                            $id = BArticle::updateArticle($post);
                            if(!$id)
                            {
                                self::getFailInfo('编辑失败',$this->method);
                            }
        ...
        if(\Yii::$app->request->isPost)
        {
            ...
            if(!empty($post['aid']))
            {
                ...
                if($article)
                {
                    ...
                    $post['wapUrl'] = 'index.php?r=wap/site/article&id='.$article->id;
                }
            }
...
//        return $this->render('industryindex');
    ...
    public function actionPositiontypeindex()
    {
         ...
         * 获取所有职位类别
         * */
        $positionr = BPositiontype::getPositionType([BPositiontype::tableName().'.iPositiontypeID' => 0]);
...
//        return $this->render('industryindex');
    ...
    public function actionCateindex()
    {
         ...
         * 获取所有证书
         * */
        $cate = EStudentcertificate::getStudentcertificate();

        return $this->renderPartial('studentcertificateindex',['cate'=>$cate]);
    }
    /**
     * Renders the index view for the module
     * @return string
    ...
    public function actionCateedit()
    {
        ...
        if(\Yii::$app->request->isPost)
        {
            ...
            if(\Yii::$app->request->post('id'))
            {
                ...
                {
                    self::getFailInfo('学员证书管理编辑失败',$this->method);
                }
            }else{
                //添加
                if(false == EStudentcertificate::insertStudentcertificate($post))
                {
                    self::getFailInfo('学员证书管理添加失败',$this->method);
                }
            }
        ...
        $dBeginTime = date('Y-m-d 23:59:59');
        return $this->renderPartial('studentcertificateedit',['cate'=>$cate,'user'=>$user,'certificate'=>$certificate,'dBeginTime'=>$dBeginTime]);
    }
}
