...
class PositionProcess
{
     ...
     * 添加-子类职位类别
     * */
    public static function addchildposition($id,$params)
    {
        /*
         * 添加子类职位类别
         * */
        ...
        {
            $iPositionID = BPosition::insertPosition(['sPositionName'=>$r['sPositionName'],'iPositionID'=>$id]);
            ...
            if(!empty($book))
            {
                if(!EPosition::batchInsertPosition($book))
                {
                    return false;
                }
            }
     ...
     * 编辑-子类职位类别
     */
    public static function editchildposition($id,$params)
    {
        //获取子类职位类别
        $position = BPosition::find()->where(['iPositionID' => $id])->all();
        if($position)
        {
        ...
        foreach($params as $k => $r)
        {
            if(!empty($r['id']))
            {
                ...
                $bookid[] = $r['id'];
                $cids = [];
                if(false == BPosition::updatePosition(['id'=>$r['id'],'sPositionName'=>$r['sPositionName'],'iPositionID'=>$id])){
                    return false;
                }
                //获取三级子类职位类别
                $ePosition = EPosition::find()->where(['iPositionID' => $r['id']])->all();
                if($ePosition)
                {
                ...
                foreach($r['child']['sPositionName'] as $key => $c)
                {
                    if(!empty($r['child']['id'][$key]))
                    {
                        ...
                        if(false == EPosition::updatePosition(['id'=>$r['child']['id'][$key],'sPositionName'=>$c,'iPositionID'=>$r['id']])){
                            return false;
                        }
                    }else{
                        //要添加的子类别
                ...
                {
                    //批量添加
                    if(!EPosition::batchInsertPosition($cbook))
                    {
                        return false;
                    }
                }
        ...
        if(!empty($ids))
        {
            ...
            foreach($diffB as $m)
            {
                ...
                //删除下面的三级子类别
                $model = EPosition::find()->where(['iPositionID'=>$m])->all();
                foreach($model as $t)
                {
                    $query = EPosition::findOne($t);
                    if(!$query)
                    {
                        return false;
                    }
