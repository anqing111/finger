...
<div class="x-body">
    <form class="layui-form" method="post" action="">
        ...
        <div class="layui-form-item">
            <label for="L_username" class="layui-form-label">专业技能：</label>
            <div class="layui-input-inline">
                <input type="text" id="sProfessionalName" value="<?=$professional['sProfessionalName'] ?? ''?>"  name="sProfessionalName" lay-verify="required" autocomplete="off" class="layui-input" maxlength="100">
            </div>
        </div>
            ...
            <div class="layui-input-inline">
                <select name="iUserID" lay-filter="iUserID">
                    ...
                    <?php foreach($user as $kc => $pre){?>
                        <option value = <?=$pre->iUserID?> <?=isset($professional['iUserID']) && $professional['iUserID'] == $pre->iUserID ? 'selected' : ''?>><?=$pre->sNick?></option>
                    <?php }?>
                </select>
            </div>
        </div>
