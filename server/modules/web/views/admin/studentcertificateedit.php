            ...
            <div class="layui-input-inline">
                <select name="iUserID" lay-filter="iUserID">
                    ...
                    <?php foreach($user as $kc => $pre){?>
                        <option value = <?=$pre->iUserID?> <?=isset($cate['iUserID']) && $cate['iUserID'] == $pre->iUserID ? 'selected' : ''?>><?=$pre->sNick?></option>
                    <?php }?>
                </select>
            </div>
        </div>
...
<script>
    ...
    layui.use(['form','layer'], function(){
        ...
        form.on('select(iUserID)', function(data){

            var user = <?=\yii\helpers\Json::encode($user)?>;
            $(user).each(function (ids,items) {
                if(parseInt(items.iUserID) == parseInt(data.value))
                {
                    $("input[name=idcard]").val(items.idcard);
                }
            });
