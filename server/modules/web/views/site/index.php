...
<?=\app\modules\web\model\process\PublicProcess::TopWeb()?>
    <div class="container index">
            ...
            <div class="swiper-container">
                <div class="swiper-wrapper">
                    <?php foreach($banner as $r){?>
                        <?php if(!empty($r->url)){?>
                            <div class="swiper-slide" style="background-image: url(<?=Yii::$app->params['imagePath'].$r->PCBanner?>);" onclick="jumpUrl('<?=$r->url?>')"></div>
                        <?php }else{?>
                            <div class="swiper-slide" style="background-image: url(<?=Yii::$app->params['imagePath'].$r->PCBanner?>);"></div>
                        <?php }?>
                    <?php }?>
                </div>
        ...
        <div class="person-show">
            ...
            <div class="content-box flex-box" style="height: 336px">
                <img src="<?=Url::to('images/person.jpg')?>" alt="" class="img-title">
                <div class="group filler">
                    <?php foreach($video as $ks => $s){?>
                        <div class="play">
                            <img src="<?= Url::to('images/play.png') ?>" alt="" class="playImg">
                            <img src="<?=Yii::$app->params['imagePath'].$s->sVideoImg?>" class="item" onclick="videoPlay('<?=$s->sVideoUrl?>')" style="height: 164px;opacity:0.9;background:rgba(0,0,0,1);position:relative;top:-40px"/>
                        </div>
                    <?php }?>
                </div>
                ...
                <div class="hot-list flex-box filler" style="width: 26%">
                    <div class="imgs">
                        <?php foreach($article4 as $k7 => $r7){?>
                            ...
                            <img src="<?=Yii::$app->params['imagePath'].$r7->picture?>" alt="" onclick="jumpUrl('index.php?r=web/site/articleinfo&id=<?=$r7->id?>')">
                        <?php }?>
                    </div>
                    <div class="list filler">
                        <div class="title flex-box">
                        ...
                        <div class="content">
                            <?php foreach($article4 as $k8 => $r8){?>
                                <?php if($k8 < 3){
                                    ...
                                    echo '<div class="item" onclick="jumpUrl(\'index.php?r=web/site/articleinfo&id='.$r8->id.'\')">
        ...
        $(function () {
            ...
            $('.play').each(function () {
                $(this).mousemove(function(){
                    ...
                    $(this).find('.playImg').css('background','rgba(0,0,0,1)');
                    $(this).find('.playImg').css('opacity','0.9');
                    $(this).find('.item').css('opacity','0.1');
                });
                $(this).mouseout(function(){
                    $(this).css('background','rgba(0,0,0,0)');
                    $(this).css('opacity','1');
                    $(this).find('.playImg').css('background','rgba(0,0,0,0)');
                    $(this).find('.playImg').css('opacity','0.1');
                    $(this).find('.item').css('opacity','0.9');
                });
            });
